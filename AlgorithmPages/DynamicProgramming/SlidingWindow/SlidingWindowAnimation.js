window.loadSlidingWindow = function () {    
    ...
    let defaultData = generateRandomList(Math.round(Math.random() * 10 + 10));
    ...
    function getColor(index) {
        if (index >= frames[currentFrame].window1 && index <= frames[currentFrame].window2)
                return 'red';
        else 
                return 'lightblue';
    ...
    function slidingWindow(arr, num) {   
        if (num > arr.length) {
          ...
          tempSum = maxSum;
          appendToExplanation(`First Window: [0, ${num-1}], equals ${tempSum}`, 0, num-1);
          ...
          for (let i = num; i < arr.length; i++) {
            ...
            appendToExplanation(`Window: [${i - num + 1}-${i}], equals ${tempSum}`, i - num + 1, i);
          }
    ...
    function updateStepLog() {
        ...
        stepLog.innerHTML += `List: ${currentData.join(", ")}<br>`; // Initial list
        ...
        if (currentFrame === frames.length - 1) {
            stepLog.innerHTML += `Stemmed from this list: ${data.join(", ")}`; // Final list
        }
    ...
    async function loadAnimation() {
        ...
        frames.push({
            data: [...data],
            explanation: "Beginning the Sliding Window Algorithm: "
        });

        // Middle frames: has highlights
        const maxSum = await slidingWindow(currentData, num);
            

        // Last frame: final sorted array, no highlights
        frames.push({
            data: [...data],
            explanation: `Maximum sum: ${maxSum}`
        });

        drawFrame(frames[currentFrame]);
    }

    function recordFrame(explanation = "", window1, window2) {
    ...
    function appendToExplanation(text, window1, window2) {
        recordFrame(text, window1,window2);
    }
