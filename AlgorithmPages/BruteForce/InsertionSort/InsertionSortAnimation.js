// Middle Display Panels Animation - Insertion Sort 
window.loadInsertionSort = function () {
    ...
    // Controls the execution of the Insertion Sort algorithm while recording frames for animation
    async function insertionSort(arr){
        for (let i = 1; i < arr.length; i++){
            pivotIndex = i;
            appendToExplanation(`Current value to insert: ${arr[i]}`);
            let key = arr[i];
            let j = i - 1;

            while (j >= 0 && arr[j] > key){
                swapIndices = [j];
                appendToExplanation(`Checking index ${j} for insertion`);
                arr[j + 1] = arr[j];
                j = j - 1;
            }
            swapIndices = [];
            arr[j + 1] = key;
            swapIndices = [j + 1];
            appendToExplanation(`Inserted ${arr[j+1]} at index ${j + 1}`);
            swapIndices = [];
        }
    }
    ...
    // Enables and contextualizes parts of top control bar relevant to Insertion Sort
    ...
    // Ties Insertion Sort animation functionality to main page
